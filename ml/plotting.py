...
def plot_data_samples(inputs, outputs, title="", sample_idx=None):
    ...
    # corresponds to a horizontal velocity field
    input_data_u = input_sample[0]
    # corresponds to a horizontal velocity field
    output_data_u = output_sample[0]
    # corresponds to a vertical velocity field
    output_data_v = output_sample[1]

    all_data = np.concatenate(
        [input_data_u.flatten(), output_data_u.flatten(), output_data_v.flatten()])
    vmin = all_data.min()
    vmax = all_data.max()

    fig, axes = plt.subplots(1, 3, figsize=(12, 5))

    im1 = axes[0].imshow(input_data_u, origin='lower',
                         cmap="RdYlBu_r", vmin=vmin, vmax=vmax)
    axes[0].set_title(f"Sample Input u (Idx: {sample_idx})")
    fig.colorbar(im1, ax=axes[0], label="Value")

    im2 = axes[1].imshow(output_data_u, origin='lower',
                         cmap="RdYlBu_r", vmin=vmin, vmax=vmax)
    axes[1].set_title(f"Sample Output u (Idx: {sample_idx})")
    fig.colorbar(im2, ax=axes[1], label="Value")

    im3 = axes[2].imshow(output_data_v, origin='lower',
                         cmap="RdYlBu_r", vmin=vmin, vmax=vmax)
