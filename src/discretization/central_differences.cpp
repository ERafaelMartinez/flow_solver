...
double CentralDifferences::computeDuvDx(int i, int j) const 
{
    ...
    const double uTopRight  = (u().at(i,j) + u().at(i,j+1)) / 2.;         // u at top right corner of cell
    const double uTopLeft  = (u().at(i-1,j) + u().at(i-1,j+1)) / 2.;      // u at top left corner of cell
    const double vTopRight  = (v().at(i,j) + v().at(i+1,j)) / 2.;         // v at top right corner of cell
    const double vTopLeft  = (v().at(i-1,j) + v().at(i,j)) / 2.;          // v at top left corner of cell
...
double CentralDifferences::computeDuvDy(int i, int j) const
{
    ...
    const double vTopRight  = (v().at(i,j) + v().at(i+1,j)) / 2.;         // v at top right corner of cell
    const double vBottomRight = (v().at(i,j-1) + v().at(i+1,j-1)) / 2.;   // v at bottom right corner of cell
    const double uTopRight  = (u().at(i,j) + u().at(i,j+1)) / 2.;         // u at top right corner of cell
    const double uBottomRight = (u().at(i,j-1) + u().at(i,j)) / 2.;       // u at bottom right corner of cell
