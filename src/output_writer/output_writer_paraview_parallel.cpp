...
OutputWriterParaviewParallel::OutputWriterParaviewParallel(
...
void OutputWriterParaviewParallel::gatherData() {
  // std::array<int,2> size, std::array<double,2> origin, std::array<double,2>
  // meshWidth
  ...
  for (int j = 0; j < jEnd; j++) {
    for (int i = 0; i < iEnd; i++) {
      ...
      u_.at(iGlobal, jGlobal) = discretization_->u().interpolateAt(x, y);
      v_.at(iGlobal, jGlobal) = discretization_->v().interpolateAt(x, y);
      p_.at(iGlobal, jGlobal) = discretization_->p().interpolateAt(x, y);
    }
  }
...
void OutputWriterParaviewParallel::writeFile(double currentTime) {
  ...
  dataSet->SetDimensions(
      nCellsGlobal_[0] + 1, nCellsGlobal_[1] + 1,
