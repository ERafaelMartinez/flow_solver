...
/* This class defines the interface for pressure solvers in the numerical
   simulation framework. It optionally receives as input a discretization scheme
   derived from SystemDiscretization and provides methods to set the
   discretization and solve the pressure equation accordingly.
*/
...
public:
   ...
   * to the pressure field in the discretization by calling the individual
   * boundary methods.
   */
  void setBoundaryConditions();

  /**
   * @brief Set the bottom boundary conditions for the pressure field.
   *
   * Applies Neumann boundary condition: p(i,0) = p(i,1)
   */
  void setBoundaryConditionsBottom();

  /**
   * @brief Set the top boundary conditions for the pressure field.
   *
   * Applies Neumann boundary condition: p(i,jmax+1) = p(i,jmax)
   */
  void setBoundaryConditionsTop();

  /**
   * @brief Set the left boundary conditions for the pressure field.
   *
   * Applies Neumann boundary condition: p(0,j) = p(1,j)
   */
  void setBoundaryConditionsLeft();

  /**
   * @brief Set the right boundary conditions for the pressure field.
   *
   * Applies Neumann boundary condition: p(imax+1,j) = p(imax,j)
   */
  void setBoundaryConditionsRight();
